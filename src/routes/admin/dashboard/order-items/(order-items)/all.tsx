...
export const Route = createFileRoute('/admin/dashboard/order-items/(order-items)/all')({
...
function RouteComponent() {
  ...
  const { isPending, data: allShippingOrderItems } = useGetOrderItemsForAdmin({
        page: pageIndex + 1,
        limit: pageSize,
      },);
 const filteredData =
    filterValue === " "
      ? allShippingOrderItems?.data
      : allShippingOrderItems?.data?.filter((item) => item.order_id.payment_status === filterValue);
